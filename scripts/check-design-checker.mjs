...
//
// scripts/fixtures/design-json/{login.json, design-book.json, registry.json, tokens.css} are
// byte-for-byte copies of the Melete fixtures at commit cadebf0 — tests/fixtures/etalons/login.json
// (the shared example, valid against that registry with exactly one proposal),
// tests/fixtures/etalons/design-book.json (the book example: light and dark fixtures, zero
// proposals, valid against that registry and vocabulary) and tests/fixtures/react-project/
...
console.log('Design-checker check passed: valid HTML and JSON page folders accepted; missing/flat link, manifest gaps, raw values, index gaps and the reserved slug rejected with named errors; every JSON code — the sixteen of the Melete validator plus INVALID-ETALON, SLUG-MISMATCH and NO-REGISTRY — rejected by name on a mutation of the shared example; the book example valid on both fixtures.');
