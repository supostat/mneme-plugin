...
//   the nineteen codes of the Melete validator, in its order and wording: DUPLICATE-ID,
//   UNKNOWN-NODE, PARENT-AFTER-CHILD, MULTIPLE-PARENTS, ORPHAN-NODE, PROPOSAL-WITHOUT-NOTE,
//   UNKNOWN-COMPONENT, REFUSED-TAG, UNKNOWN-TAG, UNKNOWN-PROP, UNKNOWN-ATTRIBUTE, BAD-ENUM,
//   FUNCTION-PROP-VALUE, RAW-STYLE-VALUE, SLOT-NOT-NODE, MISSING-PROP, UNKNOWN-TOKEN,
//   MISSING-FIXTURE-PATH, UNUSED-STATE.
//   The six primitives (Stack, Row, Text, Box, Element, Placeholder) and STYLE_PROPS are copies of
//   src/etalon/primitives.ts of Melete: a raw value in a style prop of a primitive is
//   RAW-STYLE-VALUE; registry components style themselves and are not checked for it. The four
//   child-layout props (grow, alignInParent, scroll, maxWidth) say how a node behaves inside its
//   parent, so every container carries them; only maxWidth is a style prop — the other three carry
//   a decision, not a value from the design system.
//   The element vocabulary is DATA of the registry (elements: tags, commonAttributes,
//   tagAttributes), generated by the server from the project's own React types: an Element's tag
//   must be one of them or a custom element, and every prop that is not the Element's own is an
...
const CHILD_LAYOUT_PROPS = [
  { name: 'grow', type: 'boolean', required: false },
  { name: 'alignInParent', type: 'enum', values: ALIGNMENTS, required: false },
  { name: 'scroll', type: 'boolean', required: false },
...
const STYLE_PROPS = new Set(['fill', 'stroke', 'radius', 'padding', 'gap', 'height', 'maxWidth', 'tone', 'font', 'size', 'weight', 'leading']);
const isPrimitive = (component) => PRIMITIVES.some((primitive) => primitive.name === component);
const ELEMENT_PRIMITIVE = 'Element';

// A copy of src/etalon/element-vocabulary.ts of Melete. The refusal is asked BEFORE the
// vocabulary: className and style are declared by React itself, so asking the vocabulary first
// would let a single className escape the token contract.
const REFUSED_ATTRIBUTES = new Map([
  ['class', 'styling goes through the style props bound to tokens'],
  ['className', 'styling goes through the style props bound to tokens'],
  ['style', 'styling goes through the style props bound to tokens'],
]);
const EVENT_HANDLER = /^on[A-Z]/;

// Tags an etalon never opens, whatever the project's types declare. The refusal is asked BEFORE
// the vocabulary: a real project declares svg, so a vocabulary-first order would leave this
// branch unreachable.
const REFUSED_TAGS = new Map([
  ['svg', 'an icon is a component: bind one the registry lists, or ask for it with a proposal:Icon node'],
]);
const tagRefusal = (tag) => REFUSED_TAGS.get(tag) ?? null;

const DATA_ATTRIBUTE = /^data-[a-z][a-z0-9-]*$/;
const CUSTOM_ELEMENT = /^[a-z][a-z0-9]*(?:-[a-z0-9]+)+$/;

const isKnownTag = (elements, tag) => CUSTOM_ELEMENT.test(tag) || elements.tags.includes(tag);
const isKnownAttribute = (elements, tag, name) =>
  DATA_ATTRIBUTE.test(name) || elements.commonAttributes.includes(name) || (elements.tagAttributes[tag] ?? []).includes(name);

function attributeRefusal(name) {
  const refusal = REFUSED_ATTRIBUTES.get(name);
  if (refusal !== undefined) return refusal;
  return EVENT_HANDLER.test(name) ? 'behaviour is wired in code, not in an etalon' : null;
}

const REGISTRY_REMEDY = 'start /mneme:design-server — the registry is written by the server only';
const STALE_SOURCE_HINT = 'hint: two sources can be behind this — registry.json and tokens.css are written by the design server, so a missing component or token means it has not rewritten them (start /mneme:design-server, never edit registry.json by hand); the primitives are a copy inside this checker, so a prop Melete added after the copy was taken is missing HERE, and only updating the PRIMITIVES table of plugin/scripts/check-design-etalon.mjs fixes that';
const STALE_SOURCE_CODES = new Set(['UNKNOWN-COMPONENT', 'UNKNOWN-PROP', 'UNKNOWN-TOKEN']);
...
const isElementVocabulary = (elements) =>
  isPlainObject(elements) &&
  Array.isArray(elements.tags) && elements.tags.length > 0 &&
  Array.isArray(elements.commonAttributes) &&
  isPlainObject(elements.tagAttributes) && Object.values(elements.tagAttributes).every((names) => Array.isArray(names));
const isRegistry = (registry) => isPlainObject(registry) && registry.melete === 1 && Array.isArray(registry.components) && registry.components.every(isRegistryComponent) && isElementVocabulary(registry.elements);
...
function readRegistry() {
  ...
  if (!isRegistry(registry)) {
    failures.push(`NO-REGISTRY: ${registryPath} is not a Melete registry (melete: 1 with a components list and an element vocabulary is required) — ${REGISTRY_REMEDY}`);
    return null;
  }
  return registry;
}
...
function checkProps(node, component, elements, issues) {
  const props = new Map(component.props.map((prop) => [prop.name, prop]));
  const isElement = component.name === ELEMENT_PRIMITIVE;
  if (isElement) {
    checkTag(node, elements, issues);
  }
  for (const [name, value] of Object.entries(node.props)) {
    const prop = props.get(name);
    if (prop === undefined) {
      if (isElement) {
        checkAttribute(node, elements, name, issues);
        continue;
      }
...
function checkTag(node, elements, issues) {
  const tag = node.props.tag;
  if (typeof tag !== 'string' || classifyPropValue(tag).kind !== 'literal') return;
  const refusal = tagRefusal(tag);
  if (refusal !== null) {
    issues.push({ code: 'REFUSED-TAG', message: `node "${node.id}": tag "${tag}" is refused — ${refusal}` });
    return;
  }
  if (!isKnownTag(elements, tag)) {
    issues.push({ code: 'UNKNOWN-TAG', message: `node "${node.id}": no element is called "${tag}" in this project's vocabulary` });
  }
}

function checkAttribute(node, elements, name, issues) {
  const refusal = attributeRefusal(name);
  if (refusal !== null) {
    issues.push({ code: 'UNKNOWN-ATTRIBUTE', message: `node "${node.id}": attribute "${name}" is refused — ${refusal}` });
    return;
  }
  const tag = node.props.tag;
  if (typeof tag !== 'string' || !isKnownAttribute(elements, tag, name)) {
    issues.push({ code: 'UNKNOWN-ATTRIBUTE', message: `node "${node.id}": "${typeof tag === 'string' ? tag : 'element'}" takes no attribute "${name}"` });
  }
}
  ...
  for (const node of etalon.nodes) {
    if (isProposal(node.component)) {
      ...
      if (component === undefined) {
        ...
        checkProps(node, component, registry.elements, issues);
      }
    }
...
function checkJsonEtalon(text) {
  ...
  const hint = issues.some((issue) => STALE_SOURCE_CODES.has(issue.code)) ? STALE_SOURCE_HINT : null;
  return {
